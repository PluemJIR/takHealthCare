...
export default function ComBlood(props) { 
    ...
    const getItem = async () => {
        const result = await axios.get(`http://localhost:8000/item/1`) 
        setList(result.data)
    }
    ...
    const getCriteriaById = async () => {
        ...
        if (result.data[0] !== undefined) {
            for (let i=0;i<result.data.length;i++){
                if (result.data[i].hospcode == hospcode) { 
    ...
    return (
        <div className='comBloodContainer'>
            <div><CloseCircleOutlined onClick={() => props.handleClose1()} style={closeStyle} /></div> 
