...
export default function Feces(props) { 
    ...
    const getItem = async () => {
        const result = await axios.get(`http://localhost:8000/item/4`) //เปลี่ยน
        setList(result.data)
    }
    ...
    return (
        <div className='comBloodContainer'>
            <div><CloseCircleOutlined onClick={() => props.handleClose4()} style={closeStyle} /></div> 
            <img src='./test4.png'></img>
            <br></br>
            <p style={textStyle}>ผลตรวจอุจจาระ</p>
