...
export default function Urine(props) { 
    ...
    const getItem = async () => {
        const result = await axios.get(`http://localhost:8000/item/2`) //เปลี่ยน
        setList(result.data)
    }
    ...
    return (
        <div className='comBloodContainer'>
            <div><CloseCircleOutlined onClick={() => props.handleClose2()} style={closeStyle} /></div> 
