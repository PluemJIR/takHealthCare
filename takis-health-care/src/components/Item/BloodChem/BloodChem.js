...
export default function BloodChem(props) {
    ...
    const getItem = async () => {
        const result = await axios.get(`http://localhost:8000/item/3`)
        setList(result.data)
    }
    ...
    return (
        <div className='bloodChemContainer'>
            <div><CloseCircleOutlined onClick={() => props.handleClose3()} style={closeStyle} /></div>
            <img src='./test3.png'></img>
            <br></br>
            <p style={textStyle}>ผลตรวจเลือด</p>
